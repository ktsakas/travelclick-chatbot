...
ChatBot.prototype.normalize = function (entities) {
	...
	var firstValEntities = [
		'intent', 'yes_no', 'nights', 'guests', 'roomType', 'location', 'hotelInfo'
	];
...
ChatBot.prototype.parseBook = function (text, entities, context, cb) {
	...
	if (!context.book) {
		...
		if (!context.dateIn || !context.dateOut) {
			if (dates.length == 0) {
				this.addMessage("I expected a date. Could you repeat that?")
			}

			if (!context.dateIn) context.dateIn = dates[0];

			if (!context.dateOut) {
				if (dates.length >= 2) {
					context.dateOut = dates[1];
				} else if (context.dateIn && entities.nights) {
					context.dateOut = 
						moment(context.dateIn).add(1, 'months').subtract(1, 'days').format('YYYY-MM-DD');
				}
			}
...
ChatBot.prototype.merge = function (text, entities, context, cb) {
	...
	if (context.intent == "book" || entities.intent[0].value == "book") {
		this.parseBook(text, entities, context, cb);
