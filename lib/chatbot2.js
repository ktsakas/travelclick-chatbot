...
ChatBot.prototype.merge = function (entities, context, cb) {
	...
	if (entities.datetime) {
		...
		if (entities.datetime.grain == 'month') {
			dateOut = moment(context.dateIn).add(1, 'months').subtract(1, 'days').format('YYYY-MM-DD');
		}

		if (!entities.dateIn && dateIn) context.dateIn = dateIn;
		if (!entities.dateOut && dateOut) context.dateOut = dateOut;
	}

	if (entities.number) {
		if (!context.nights) context.nights = entities.number[0].value;
		if (!context.guests) context.guests = entities.number[0].value;
	}
