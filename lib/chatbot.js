...
ChatBot.prototype.updateParams = function (params) {
	...
	var parsed = _.clone(params);

	if (guests != "") {
		parsed['roomType'] = roomTypes[guests.number - 1];
	} else if (roomType != "") {
		if (roomType.match(/single/i)) {
			parsed['guests'] = { number: 1 };
		} else if (roomType.match(/double/i)) {
			parsed['guests'] = { number: 2 };
		} else if (roomType.match(/triple/i)) {
			parsed['guests'] = { number: 3 };
		}
	}

	if (datePeriod != "") {
		var datePeriod = params['datePeriod'].split('/');
		parsed['dateIn'] = datePeriod[0];
		parsed['nights'].number = moment(datePeriod[1], "YYYY-MM-DD")
			.diff(parsed['dateIn'], 'days');
		// console.log(moment(parsed['dateIn'], "YYYY-MM-DD").toString(),
		...
		parsed['datePeriod'] = dateIn + "/" + dateOut;
	}

	return _.isEqual(params, parsed) ? false : parsed;
};
...
ChatBot.prototype.setupActions = function (message) {
	...
	this.ai.parse('availability', function () {

	});

	this.ai.on('availability', (params) => {
		var newParams= this.parseAvailParams(params);
		console.log('updated parameteres', params, newParams);
		if ( newParams ) {
			this.ai.query(message, newParams);
			return;
		}

		params = Availability.parseOptions(params);
		// Update the parameters after parsing
		this.ai.setParams(params);

		console.log("availability action called");
	...
	this.ai.on('rooms-info', (params) => {
		RoomAmenities.getRooms(1098, 'tv', function (err, rooms) {
			...
			if (rooms.length == 0) {
				self.addAnswer({
					type: "msg",
					text: "Unfortunately, we don't have any rooms with wifi."
				})
			} else {
				self.addAnswer({
					type: "msg",
					text: "Only, some of our rooms have wifi: "
				});
...
var chatb = new ChatBot();
console.log("updated");
console.log(chatb.updateParams({
	guests: {number: 1},
	// roomType: 'single',
	dateIn: '2016-07-05',
	nights: {number: 5},
	datePeriod: '2016-07-05/2016-07-10'
}));
